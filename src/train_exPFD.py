...
class ClassifyModel(customizedModule):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sentence_mask=None, label=None, ):
        ...
        sentence_mask_full = torch.stack([torch.cat((torch.cat((i[0, :], self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, i[1, :], self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, i[2, :], self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, i[3, :]), 0)), 1) for i in (h_mask == v_mask)])
        ...
        section_mask_full = torch.stack([torch.cat((torch.cat((i[0, :], self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, i[1, :], self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, i[2, :], self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, i[3, :]), 0)), 1) for i in attention_mask_2])
        # *************************************************************************************************************************************************
        # 解耦图池化模块*******
        section_mask, sentence_mask, new_att1, new_sec_fea1 = self.DGP[0](token_feature,pooled_feature.view(-1, 4, 768),section_mask_full, sentence_mask_full)
...
def main():
    ...
    if not args.trained_model:
        ...
        model = torch.load('/home/ltf/code/data/best_weight/exPFD/2021-05-09_13-47-43.pt')   #best weight for exMeSH
