...
class ClassifyModel(customizedModule):
    def __init__(self, pretrained_model_name_or_path, args, device, is_lock=False):
        ...
        self.DGP.append(DecoupledGraphPooling(1020, 768, 0.1))
        self.DGP.append(DecoupledGraphPooling(510, 768, 0.1))
        self.DGP.append(DecoupledGraphPooling(255, 768, 0.1))
        self.classifier = nn.Linear(768, self.args.num_labels)
        self.zero = torch.zeros((510, 510)).to(device)
        if is_lock:
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sentence_mask=None, label=None, ):
        ...
        token_feature = torch.reshape(token_feature,(-1, 5 * (self.args.max_seq_length - 2), 768))  # batch * word num * feature dim
        # ******************************************************************************************************************************************************
        # 生成mask
        h_mask = sentence_mask.unsqueeze(-1).expand(self.args.train_batch_size, 5, self.args.max_seq_length - 2,self.args.max_seq_length - 2)
        v_mask = sentence_mask.unsqueeze(-2).expand(self.args.train_batch_size, 5, self.args.max_seq_length - 2,self.args.max_seq_length - 2)
        sentence_mask_full = torch.stack([torch.cat((torch.cat((i[0, :], self.zero, self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, i[1, :], self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, i[2, :], self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, i[3, :], self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, self.zero, i[4, :]), 0)), 1) for i in (h_mask == v_mask)])
        attention_mask_1 = torch.tensor(attention_mask.view(-1, 5, self.args.max_seq_length).unsqueeze(-1),dtype=torch.float)
        attention_mask_2 = torch.matmul(attention_mask_1, attention_mask_1.transpose(2, 3))[:, :, 1:-1, 1:-1]
        section_mask_full = torch.stack([torch.cat((torch.cat((i[0, :], self.zero, self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, i[1, :], self.zero, self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, i[2, :], self.zero, self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, i[3, :], self.zero), 0),
                                                     torch.cat((self.zero, self.zero, self.zero, self.zero, i[4, :]), 0)), 1) for i in attention_mask_2])
        # *************************************************************************************************************************************************
        # 解耦图池化模块*******
        section_mask, sentence_mask, new_att1, new_sec_fea1 = self.DGP[0](token_feature,pooled_feature.view(-1, 5, 768),section_mask_full, sentence_mask_full)
...
def main():
    ...
    if not args.trained_model:
        ...
        model = torch.load('/home/ltf/code/data/best_weight/exMeSH/2021-05-13_21-07-58.pt')   #best weight for exMeSH
