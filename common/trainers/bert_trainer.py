...
class BertTrainer(object):
    def __init__(self, model, optimizer, processor, scheduler, tokenizer, args):
        ...
        self.best_dev_f1,self.best_test_f1, self.unimproved_iters = 0, 0,0
    ...
    def train_epoch(self, train_dataloader):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            input_ids, input_mask, segment_ids, sentence_mask, label_ids = batch
    ...
    def train(self):
        if self.args.is_hierarchical:
            train_features = convert_examples_to_features_hs(
                self.train_examples, self.args.max_seq_length, self.tokenizer)
            # train_features = np.load('/home/ltf/code/data/SDC/exPFD/exPFD_train_TKDE_4.npy', allow_pickle=True).tolist()
        else:
            train_features = convert_examples_to_features_ns(
                self.train_examples, self.args.max_seq_length, self.tokenizer)
        ...
        unpadded_sentence_mask = [f.sentence_mask for f in train_features]
        unpadded_label_ids = [f.label_id for f in train_features]
        ...
        label_ids = torch.tensor(unpadded_label_ids, dtype=torch.long)
        assert len(padded_input_ids) == len(padded_input_mask) == len(padded_segment_ids) == len(padded_sentence_mask)== len(label_ids)

        train_data = TensorDataset(padded_input_ids, padded_input_mask, padded_segment_ids, padded_sentence_mask, label_ids)
        ...
        for epoch in trange(int(self.args.num_train_epochs), desc="Epoch"):
            ...
            tqdm.write(self.log_template.format(epoch + 1, self.iterations, epoch + 1, self.args.num_train_epochs,
            ...
            if dev_f1 > self.best_dev_f1:
                ...
                tqdm.write("Early Stopping. Epoch: {}, Best Dev F1: {}, Best Test F1: {}".format(epoch, self.best_dev_f1, self.best_test_f1))
